...
Demographic data for caribou populations, including abundance, vital rates and population growth, are most often obtained through the tracking of caribou cows marked with telemetry collars [e.g. @courtois_aerial_2003; @rettie_dynamics_1998;@hervieux_widespread_2013;@johnson_science_2020]. Caribou cows are typically captured via net-gun from aircraft in late winter, and affixed with collars carrying  Very High Frequency (VHF) and/or Global Positioning Satellite (GPS) transmitters allowing remote detection and data retrieval [see @nbckc_nbckc_2021]. Because these collars send out a mortality signal when no longer active after a predetermined period of time, adult survival can be estimated by tracking the fate of marked individuals. See @kaplan_nonparametric_1958; @pollock_survival_1989 for original approach and @mcloughlin_declines_2003; @hervieux_widespread_2013; @eacker_web-based_2019 for examples of application to caribou. Male caribou are not frequently collared. 

Collared females are also used in most estimates of recruitment. Recruitment surveys involve locating collared individuals by aircraft and then surveying the entire group of caribou to estimate the ratio of calves to cows in the group [@nbckc_nbckc_2022]. Because this estimate is based on more than just the collared sample of cows, the sample size for recruitment estimates is typically larger than for survival (see multiplier parameter $w$, cowMult below). Note that occasionally young bulls can be misidentified as cows (parameter $u$ below; refs? ), and there is also a possibility that some calves may be missed during aerial composition surveys (parameter $z$ below). We acknowledge additional uncertainty and potential bias due to misidentification of sex or missing calves with a correction term derived from a simple model of the recruitment survey observation process (parameter $c$; see Supplement B and below for details). Telemetry and aerial survey data from these methods can be combined in a simple recruitment-mortality population model [@hatter_moose_1991] that serves as the basis for monitoring population change of many caribou herds [e.g. @decesare_estimating_2012; @serrouya_comparing_2017; @eacker_web-based_2019].
...
deployments ($o=4$). We focus on exploring a set of scenarios in which
...
increases by 1% per year from a variety of starting points (Figure \@ref(fig:figDisturbance)).
Scenarios in which anthropogenic disturbance does not increase over time
are included in Supplement D.
...
To investigate the effectiveness of various monitoring strategies for reducing uncertainty about population viability, and to show how this varies with landscape condition, we begin with a method for simulating local population dynamics and monitoring. This method produces example population trajectories for which the true state is known, allowing us to assess the performance of our proposed method for integrating local demographic observations with national disturbance-demographic relationships in a Bayesian population model.
...
biases (Supplement B) and (optionally) delayed age at first reproduction
[@eacker_web-based_2019; @decesare_estimating_2012]:
\begin{equation}
...
Following Eacker et al. [-@eacker_web-based_2019], the observed number
...
@stewart_climate-informed_2023; @dyson_existing_2022] and Gaussian
distributed random year effect [@eacker_web-based_2019]:
\begin{equation}
...
misidentification biases (Supplement B), and age of first reproduction
[@eacker_web-based_2019; @decesare_estimating_2012] to get expected
recruits per female:
\begin{equation}
...
The composition survey bias correction term $c$ depends on the apparent
number of adult females per collared animal $w$, the ratio of young
bulls to adult females $q$, the probability of misidentifying young
bulls as adult females and vice versa $u$, and the probability of
missing a calf $z$ (Supplement B). To maintain consistency with Johnson
...
Also following Eacker et al. [-@eacker_web-based_2019], Kaplan-Meier
...
As in the national demographic model, we model survival probability as an adjusted function
of buffered anthropogenic disturbance $A_t$ with a log link
[@johnson_science_2020; @stewart_climate-informed_2023;
@dyson_existing_2022] and Gaussian distributed random year effect
[@eacker_web-based_2019]:
\begin{equation}
...
We assume a simple two-class demographic model, with a census in late
...
growth over a period of $y$ years, calculated as
$\bar{\lambda}_{y,t}=\sum_{x=t-y+1}^{x=t}\lambda_x$. The initial
post-juvenile population size $N_0$ is 1000 animals. For reference, all
variables and parameters are defined in Table A1.
...
TO DO: \* note distribution of random effect parameters are truncated to
...
Ensure notation consistency amongst manuscript, UI, documentaiton and
...
caribou and other endangered species, it is important to know whether a
...
Bayesian model to 200 example population trajectories (e.g. Figure
\@ref(fig:exampleTrajectories)). The probability of correct status
assessment for that scenario is the percentage of these 200 examples in
which the Bayesian posterior mean population growth rate and the true
population growth rate are both above or below 1.

## User interface and open, flexible, reuseable implementation

To make this work more user friendly and accessible to decision makers
we built a user interface that allows for the exploration of different
monitoring and anthropogenic disturbance scenarios. We built the user
interface as an R Shiny app [@chang_shiny_2023], building off and expanding an
app published by Eacker et al. [-@eacker_web-based_2019]. The app allows
users to provide existing local data or simulate data based on national
...
model. Many parameters controlling the Bayesian model, local data
simulation and antropogenic scenario are exposed in the app with
sensible defaults set, such that novice users can get reasonable results
quickly while advanced user can customise the projections to suite their
needs. We have also made the R code needed to run the app and carry out
the above analysis available as an R package, caribouMetrics. Making our
code available as an R package ensures that it is flexible and can be
re-used or expanded upon in future projects or combined with other tools
and packages (eg SpaDES or SyncroSym). The caribouMetrics package is
available on GitHub at <https://github.com/LandSciTech/caribouMetrics>
and a package website with detailed documentation is also available
(<https://landscitech.github.io/caribouMetrics/>).

# Results

```{r figStatusErrors, fig.cap="The probability of a correct status assessment varies with the amount of anthropogenic disturbance (columns), the number of years projected (rows, starting in 2023), and monitoring effort (see Figure \\@ref(fig:figDisturbance) for scenarios). Probabilities are calculated for 200 example population trajectories (see example trajectories in Figure \\@ref(fig:exampleTrajectories)). For each example population, the status assessment is correct if the Bayesian posterior mean and the true population growth rate are both less than or greater than one. When disturbance is very high the probability of correct status assessment is high even without local monitoring. Where disturbance is moderate, increasing the number of years of monitoring and the number of collars reduces uncertainty, but there are diminishing returns on additional monitoring as sample size increases.", cache=TRUE}
knitr::include_graphics(here::here("figs/forPaper/powercmult6ay3aSf1.png"))
```

# Discussion

Will need a section on the fundamental assumptions of these
models/aspatial nature of them, e.g. that collared cows and cows
observed in surveys are a random representative sample of the larger
population, and that there is full mixing in the population so that
estimated survival/recruitment rates apply equally to all animals in the
population. We can discuss this with some contrasting examples, e.g.
collaring 30 animals from a very small/restricted population (like
Little Smoky, 308,606ha, pop size estimate 78 as of COSEWIC 2014) would
likely be much more representative of the population vs. collaring 30
animals from a relatively large/expansive population (northern Quebec
range Q6, 62,156,186 ha, pop size 9000 as of COSEWIC 2014). But because
of the way this model is built, we assume that the sample is equally
representative in either of those cases, so changing the initial
starting N won't affect model performance/predictions much here...

On a related topic to population size; Note that the stuff we got from
ArtR regarding simulations for sample size does acknowledge that pops of
different sizes have different monitoring requirements, and Sara's
Labrador numbers also show targets relative to pop size... Since this
kind of aspatial model will mostly be using small population sizes in
order to see any effects, we'll jsut have to acknowledge that a caveat
of this is that we're not fully able to explore all the population size
effects with this model, but generally Josie has found little impact of
initial population size to the results.

Include some commentary on addressing limitations of aspatial
demographic models with spatial models. Not a panacea. It is possible
(cite examples) to develop spatially explicit models of movement and
adult mortality risk - though sample size is a challenge. Recruitment,
which is more variable among ranges and with anthropogenic disturbance,
is difficult to characterize at finer spatial scales because it depends
on the conditions experienced by mothers over years as they move across
the range, and on spatial variation in calf mortality risk that is not
measured directly.

We should likely also have a pgph about costs of monitoring - it'll be
pretty short, cause basically we have no idea what it costs since nobody
will tell us :) Through the NBCKC Monitoring Working Group work, we
tried over and over to get some basic cost estimates from P/T reps and
practitioners, and hit a wall - nobody wanted to tell us a thing. Don't
know if we'd now find anyone is more open to sharing, but don't think we
should get our hopes up. But at the very least we'll want to acknowledge
that we haven't accounted for relative costs of monitoring... [e.g.
costs outlined in the Telemetry guidance report from the MonitoringWG:
"Expenses to consider in radio-collared monitoring will vary, based on:
helicopter (or other aircraft) use, purchase of tracking collars, data
fees associated with the collars (the cost of which varies by operating
system, recording frequency and data transmission frequency), and
live-capture equipment (3 or more sets of capture nets and associated
buckets, a netting gun, and ankle hobbles)"] --\>perhaps the keen
MonitoringWG contacts would be willing to provide even really rough cost
estimates, and/or even jsut a relative sense of how different it would
be to e.g. monitor a few pops for many yrs vs lots of pops for a short
time...?

Re paragraph on costs of monitoring. The reality is that costs vary with
remoteness and the difficulty of finding animals to collar (which also
depends on how many collars are deployed). Since costs are so
context-specific, there isn't a good way to incorporate them into this
general analysis. It would be interesting to work through a case study
(i.e. Newfoundland) informed by context-specific costs, and consider how
incorporation of those costs may change the relative value of
alternative monitoring scenarios.

Biases in composition survey estimates also vary with vegetation cover
(i.e. the difficulty of seeing animals) and the care taken to correctly
identify the sex of unknown animals. Misidentification rates could be
estimated from discrepancies among results from multiple independent
observers analyzing photos of animal groups.

The tool we have developed is a proof of concept, designed to
demonstrate possibilities and help users develop a general understanding
of the relative value of alternative monitoring strategies for boreal
caribou. In current form the tool will have limited value for managers
who have some local data and wish to know the value of collecting more,
because it is designed to either integrate real local data or simulated
local data, but not both. Development of a tool designed to help
decision makers assess the relative value of strategies for additional
monitoring given their existing local data and national-demographic
disturbance relationships would be a useful followup to this work.

A tool that integrates real local data with national demographic
disturbance relationships can also be used to provide consistent and
transparent assessments of the strength of evidence for local deviations
from the expected mean of national demographic-disturbance
relationships. Help distinguish real variation among populations from
apparent variation that can be attributed to lack of local data.
